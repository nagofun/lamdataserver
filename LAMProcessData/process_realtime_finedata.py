...
from lamdataserver.settings import BASE_DIR, REAL_DATABASE, REAL_DATABASE
import pymysql
...
class Realtime_FineData():
	auth = apps.get_app_config('LAMProcessData')
	allWorksection_list = Worksection.objects.filter(available=True).order_by('code')
	# Get_StatusID_By_WC_ID_Dict = {}
	StatusData_Name_List = [
		'Promg_Name',
		'CNC_State',
		'CNC_G11',
		'CNC_G12',
	...
	# allWorksection_tablename_list = []
	# db = pymysql.connect("192.168.1.117", 'root', '1234QWER', 'opctags', port=3306, charset='utf8')
	try:
		db = pymysql.connect(REAL_DATABASE['HOST'],
		                     REAL_DATABASE['USER'],
		                     REAL_DATABASE['PASSWORD'],
		                     REAL_DATABASE['NAME'],
		                     port = int(REAL_DATABASE['PORT']),
		                     charset = 'utf8')
		Cursor = db.cursor()
	except:
		print('Remote Database Connect Failed')
	
	# 得到状态实时刷新表中各WC所属的记录ID
	# 确保状态实时刷新表中各WC都有记录
	for _wc in allWorksection_list:
		try:
			status_data_id = Process_Realtime_LastStatusData.objects.get(work_section = _wc).id
		except:
			_new_realtime_statusdata_obj = Process_Realtime_LastStatusData.objects.create(
				work_section = _wc,
				acquisition_timestamp = int(time.time())
			)
			_new_realtime_statusdata_obj.save()
			status_data_id = _new_realtime_statusdata_obj.id
		# Get_StatusID_By_WC_ID_Dict[_wc.id] = status_data_id
		
	def __init__(self):
		pass
	
	@classmethod
	def add_processRecord(cls):
		YearMonth_str = datetime.datetime.now().strftime('%Y%m')
		realtime_model = cls.auth.get_model('Process_Realtime_FineData_%s'%YearMonth_str)
		_timestamp = int(time.time())
		
		for _wc in cls.allWorksection_list:
			tablename = _wc.realtime_tablename
			...
			try:
				...
				# 状态实时刷新表
				_Update_StatusData_Dict = {row[1]: row[3] for row in results if row[1] in cls.StatusData_Name_List}
				_Insert_Realtime_Data_Dict = {row[1]: row[3] for row in results if row[1] in cls.RealtimeData_NAME_List}
				_Update_StatusData_Dict['acquisition_timestamp'] = _timestamp
				Process_Realtime_LastStatusData.objects.filter(work_section=_wc).update(**_Update_StatusData_Dict)
				
				# 状态记录表
				_realtime_obj = realtime_model.objects.create(
					# 工段
					work_section=_wc,
					# 获取的时间戳
					...
					# 氧含量
					oxygen_value=_Insert_Realtime_Data_Dict['O2'],
					# 激光功率
					laser_power=_Insert_Realtime_Data_Dict['OutPutPower'],
					# 机床工件坐标
					X_value=_Insert_Realtime_Data_Dict['X_OBJ'],
					Y_value=_Insert_Realtime_Data_Dict['Y_OBJ'],
					Z_value=_Insert_Realtime_Data_Dict['Z_OBJ'],
					# 机床机械坐标
					X_P_value=_Insert_Realtime_Data_Dict['X_P'],
					Y_P_value=_Insert_Realtime_Data_Dict['Y_P'],
					Z_P_value=_Insert_Realtime_Data_Dict['Z_P'],
					# 数控机床状态
					CNC_State=_Insert_Realtime_Data_Dict['CNC_State'],
					# 层提升高度
					CNC_P191=_Insert_Realtime_Data_Dict['CNC_P191'],
					# 扫描速率
					ScanningRate_value=_Insert_Realtime_Data_Dict['CNC_V'],
					# 进给率
					FeedRate_value=_Insert_Realtime_Data_Dict['CNC_v_rate'],
					# 头气流量L/min
					processing_end_flow=_Insert_Realtime_Data_Dict['TQ_BACK'],
				)
				_realtime_obj.save()
			except:
				print('Query Data From RemoteDatabase Failed.')
	...
	def getFineDataList_ByWSID(cls, WSID, starttimestamp, endtimestamp):
		...
		_finedata_list = realtime_model.objects.filter(qset).order_by('acquisition_timestamp')
...
class AnalyseData():
	...
	@classmethod
	def Make_CNCData_ZValue_AccumulateData_UsingSQL(cls, MissionObj, ifgetZValue):
		# t1=time.time()
		_mission = MissionObj
		_mission_id = _mission.id
		_worksection_id = _mission.work_section.id
		_mission_timecut = _mission.Mission_Timecut
		_FineDataModel = Realtime_FineData.getFineDataModel_ByWSID(str(_worksection_id))
		...
		firstfinedata_timestamp = _FineDataModel.objects.filter(qset & Q(if_exec_intr=1)).aggregate(timestamp=Min('acquisition_timestamp'))['timestamp']
		object_ZValue_list = list(_FineDataModel.objects.filter(qset& Q(acquisition_timestamp__gte=firstfinedata_timestamp)).extra(select={'minuteindex':"FLOOR((acquisition_timestamp-%d)/60)"%firstfinedata_timestamp}).values('minuteindex').annotate(Min('Z_value')))
		# 功率值大于100时计入累积功率，出光秒数
		object_Laser_list = list(_FineDataModel.objects.filter(qset& Q(acquisition_timestamp__gte=firstfinedata_timestamp)& Q(laser_power__gte=-20)).extra(select={'minuteindex':"FLOOR((acquisition_timestamp-%d)/60)"%firstfinedata_timestamp}).values('minuteindex').annotate(Count('id'),Sum('laser_power')))
		'''
		SELECT
			(FLOOR((acquisition_timestamp-1571535027)/60)) AS `minuteindex`, \
			sum(laser_power) as laser_sum,\
			sum((if(laser_power>0,1,0))) AS `laser_on`, \
			sum((if(laser_power>0,0,1))) AS `laser_off` \
		FROM `LAMProcessData_process_realtime_finedata_by_worksectionid_1` \
		WHERE \
			(`LAMProcessData_process_realtime_finedata_by_worksectionid_1`.`acquisition_timestamp` >= 1571532180 \
			AND `LAMProcessData_process_realtime_finedata_by_worksectionid_1`.`acquisition_timestamp` <= 1571963760 \
			AND `LAMProcessData_process_realtime_finedata_by_worksectionid_1`.`acquisition_timestamp` >= 1571535027) \
		group by minuteindex
		'''
		# 分钟时间戳-Z值列表
		minute_ZValue_list = [(i['minuteindex'], i['Z_value__min']) for i in object_ZValue_list]
		# 分钟时间戳-能量累积 & 关光秒数
		minute_Laser_list = [(i['minuteindex'], i['laser_power__sum'], i['id__count'] ) for i in object_Laser_list]
		'''3. 计算每分钟Z高度的变化值'''
		...
		_product_code = ','.join(map(lambda p: p.product_code, _mission.LAM_product.all()))
	
		ZValueData = [(_mission_id, _product_code, i[0], i[1], _P191_list[id] ) for id, i in enumerate(minute_ZValue_list)]
		'''6. 将开光功率累积、关光秒数累积数值打包成json'''
		'''[[MissionID, ProductCode, MinuteIndex, SUM_Power, LaserOFF_Seconds, LaserON_Seconds], [], [], ...]'''
		LaserData = [(_mission_id, _product_code, i[0], i[1], 60-i[2], i[2]) for id, i in enumerate(minute_Laser_list)]
	...
	# 		ZValue_list = list(
	# 			filter(lambda i: i != None, [_data.Z_value for _data in finedata if _data.laser_power and _data.laser_power > -2]))
	...
	# 		# 得到1分钟内激光功率累积、停光秒数
	# 		sum_power = sum([_data.laser_power if _data.laser_power and _data.laser_power > 100 else 0 for _data in finedata])
	# 		laseroff_seconds = sum([0 if _data.laser_power and _data.laser_power > 100 else 1 for _data in finedata])
	# 		laseron_seconds =  sum([1 if _data.laser_power and _data.laser_power > 100 else 0 for _data in finedata])
	...
	# 	print('t8-t7:%.4f'%(t8-t7))
	# 	print('t9-t8:%.4f'%(t9-t8))
	# 	print('t10-t9:%.4f'%(t10-t9))
	# 	return return_json
	...
	def AnalyseData_ZValue_ByMissionID(cls, MissionID):
		...
		if len(_Process_CNCData_Set) == 0:
			# 如数据库中无此条目，则需从FineData中计算得来
			return_json = cls.Make_CNCData_ZValue_AccumulateData_UsingSQL(_mission, True)
	...
	def AnalyseData_AccumulateData_ByMissionID(cls, MissionID):
		...
		if len(_Process_CNCData_Set) == 0:
			# 如数据库中无此条目，则需从FineData中计算得来
			return_json = cls.Make_CNCData_ZValue_AccumulateData_UsingSQL(_mission, False)
